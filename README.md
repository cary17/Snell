...
手动工作流 **Test Alpine with glibc** 使用正式 `Dockerfile` 验证四个系列的代表版本及可用架构，不推送镜像。正式发布工作流由每日新版本检测或维护者手动触发，普通代码提交不自动构建。官方 v4.1.1 arm64 包存在自报 v4.1.0 的标注差异，版本验收仅对已核对的该平台、该版本和二进制 SHA-256 做精确兼容；其他版本、架构或内容仍严格检查。
