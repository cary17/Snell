...
# 优化IP获取：优先接口公网IP，若无则通过公网服务查询；IPv6若存在则生成双份Surge配置
# 支持多种 Linux 发行版和架构
...
# 获取宿主机 IPv4 地址（优先接口公网 IPv4，若无则通过公网查询）
get_host_ipv4() {
    # 1. 查找接口上第一个非回环、非私有的 IPv4 地址
    local pub_ip=$(ip -4 addr show scope global | grep -v ' lo' | grep -oP '(?<=inet\s)\d+(\.\d+){3}' \
        | while read ip; do
    ...
    public_ip=$(curl -s -4 --connect-timeout 3 ip.sb 2>/dev/null)
    if [ -n "$public_ip" ]; then
        echo "$public_ip"
        return 0
    fi

    # 3. 公网查询失败，退回使用接口第一个私有地址（至少能工作）
    local fallback=$(ip -4 addr show scope global | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -1)
    if [ -n "$fallback" ]; then
        echo "$fallback"
        return 0
    fi
...
# 获取宿主机 IPv6 全球单播地址（如果存在）
get_host_ipv6() {
    # 查找 scope global 且地址以 2/3 开头的第一个 IPv6（排除 link-local）
    local ipv6=$(ip -6 addr show scope global | grep -oP '(?<=inet6\s)[0-9a-f:]+(?=/)' \
...
# 显示配置信息（精简版，IPv4/IPv6 双份 Surge 配置）
show_full_config() {
