...
运行阶段使用官方 `alpine:latest`；GitHub 构建设置 `pull: true`，每次实际构建都拉取当时最新正式版，不使用 `edge`。每天北京时间 03:00 定时检测官方版本：发现没有成功构建记录的新版本时，自动执行脚本校验、候选镜像构建和实际产物验收，全部通过后才发布正式标签；已有版本不重复构建。普通代码推送与 Alpine 更新不触发构建；维护者仍可手动指定版本，并用 `force` 重建已有版本。
...
仓库包路径需匹配 `Version/vX.Y.Z/snell-server-vX.Y.Z-linux-ARCH.zip`。版本号以下载时的压缩包文件名及镜像 `/snell-version` 为准；二进制 `--version` 输出仅用于确认程序可执行，不参与版本判定。构建会验证 ZIP 可解压，并把实际使用文件的 SHA-256 写入镜像 `/snell-archive-sha256`；发布工作流还会按平台写入 `.build-records/版本.txt`。SHA-256 仅用于来源记录，不作为需要人工同步的构建门禁，也不等同于上游签名验证。

仓库每天北京时间 03:00 同步官网正式版二进制。官网文件 SHA-256 变化或出现新正式版时，自动验证 ZIP 与 ELF 架构并更新 `Version/vX.Y.Z/`；测试版不会保存到仓库。归档同步提交本身不触发镜像构建；每日版本检测若发现该新版本尚无成功构建记录，则自动进入校验、候选构建和发布流程。构建完成状态只记录在 `.build-records/版本.txt`，不再维护单独的版本列表。
...
`--dry-run` 仅支持安装和重配置；更新、卸载、启停等操作带此参数时会报错退出，不执行操作。
...
工作流先以 BuildKit 的无标签、按 digest 推送候选制品到 GHCR，以及已配置凭据时的 Docker Hub；同一候选 digest 在两个仓库分别核对。每个平台随后验证 Alpine/运行库来源、镜像内版本、原归档 SHA-256、非 root、启动、配置和正常停止；amd64 的 v3-v5 额外做普通/HTTP 混淆的 TCP、IPv6、UDP 与受支持的自定义 DNS 转发检查。候选摘要只用于定位验证对象，不能作为用户标签拉取。

所有验证通过后，发布脚本再次查询官网、校验输入及两个仓库的候选摘要，才以已验证 digest 写入完整版本标签；当前版本高于或等于同大版本已知最高版本时附加主版本标签（如 `v6`），仅全局官网最新版附加 `latest`。手动构建旧版本只发布完整版本。官方版本查询、输入或任一候选摘要检查失败时，零正式标签变化，也不记录成功构建。两个注册表之间不存在原子事务：推广开始后的网络写入故障可能使先完成的仓库已更新，但脚本会失败且不写成功记录，不会声称可自动回滚。

主版本标签（`v3`、`v4`、`v5`、`v6`）比较当前构建版本、`.build-records/` 中已成功发布的版本和 `Version/v*` 中仓库备份版本，不依赖官网是否仍提供该大版本。当前版本高于或等于同大版本已知最高版本时才更新 `vN`；相同基础版本下正式版高于测试版。手动工作流 **Repair Snell major tags** 从成功构建记录选出每个大版本最高版本，再读取注册表中对应具体版本 tag 的当前 digest；两个仓库全部预检通过后才回填 `vN`，不重建镜像、不改具体版本标签。

手动工作流 **Test Alpine with glibc** 使用正式 `Dockerfile` 验证四个系列的代表版本及可用架构，不推送镜像。正式发布工作流由每日新版本检测或维护者手动触发，普通代码提交不自动构建。各版本以官方压缩包文件名及镜像 `/snell-version` 为准；二进制内置 banner 可能滞后，仅验证 `--version` 命令可正常执行。
