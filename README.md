...
所有验证通过后，发布脚本再次查询官网、校验输入及两个仓库的候选摘要，才以已验证 digest 一次性写入每个仓库的完整版本标签；仅系列最新版附加主版本标签（如 `v6`），仅全局最新版附加 `latest`。手动构建旧版本只发布完整版本。官方版本查询、输入或任一候选摘要检查失败时，零正式标签变化，也不记录成功构建。两个注册表之间不存在原子事务：推广开始后的网络写入故障可能使先完成的仓库已更新，但脚本会失败且不写成功记录，不会声称可自动回滚。
