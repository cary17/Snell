...
此前发布 run `35572684921` 已成功完成 v6 rc2 的 amd64/386/arm64 校验并发布。随后矩阵 run `35597520048` 仅报告一个失败：v4.1.1/arm64 的上游二进制自报 v4.1.0，尚未进入该项运行场景。
对官方下载的 `snell-server-v4.1.1-linux-aarch64.zip` 做了静态来源比对，没有执行本地二进制：官方ZIP与仓库ZIP均为 `38d4cdc03dcdb3608af8594df83e1795265167fafc5d802f815148908902d758`；二进制SHA-256均为 `a6dceb898ade6da58840bf26499a0747894fb1c6407878139c8d863e7926d297`。仓库v4.1.0/arm64二进制为 `f3e87f63ecf039fb83e4e56c4796bccd10e8ae9ecb2b66bb452a12625821ee69`，并非误用了旧包。
该标注兼容限定为请求v4.1.1、linux/arm64、上述二进制摘要且自报v4.1.0；不放宽其他稳定版或预发布版检查。新增断言仅供日后手动GitHub验收，收尾修改后没有重跑构建或测试，不把旧矩阵记成全部通过。
...
历史事实：GitHub run `35541323005` 的 job `106159518603` 构建候选摘要 `sha256:ef2ced8ce838d674153faed9848b0c46b96e1bdd19ebd3363225f688558692be` 后，旧断言要求 `snell-server v6.0.0rc2 (`，而官方二进制实际自报 `snell-server v6.0.0 (`，因此在 amd64 运行库元数据输出后失败。该次旧流程已在验证前推送完整版本标签；这属于已替换的历史行为。当前校验仍严格匹配 `/snell-version`，但请求预发布版时允许同一三段正式核心版本的自报。
